...
class Art {
  ...
  static async list(ctx: ZYContext, next: Next) {
    ...
    console.log(`---------`);
    let articleT = tables.article

    let articleList = await articleT.findAll({
      attributes:["id", "user_id","title", "views", "cover", "digest"],
      limit: 10,
      offset:0
    })
  ...
  static async details(ctx: ZYContext, next: Next) {
    ...
    let article = await articleT.findOne({
      where:{
        id: 1
      }
    })
